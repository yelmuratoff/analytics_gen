  ...
  group('Analytics Plan Tests', () {
    ...
    group('auth', () {
      test('logAuthLogin constructs correctly', () {
        expect(() => analytics.logAuthLogin(method: 'test'), returnsNormally);
      });
      test('logAuthLoginV2 constructs correctly', () {
        expect(
          () => analytics.logAuthLoginV2(
      ...
      test('logAuthLogout constructs correctly', () {
        expect(() => analytics.logAuthLogout(), returnsNormally);
      });
      test('logAuthPhoneLogin constructs correctly', () {
        expect(
          () => analytics.logAuthPhoneLogin(
            phoneCountry: 'test',
            trackingToken: 'test',
          ),
          returnsNormally,
      ...
      test('logAuthSignup constructs correctly', () {
        expect(() => analytics.logAuthSignup(method: 'test'), returnsNormally);
      });
      test('logAuthVerifyUser constructs correctly', () {
        expect(
          () => analytics.logAuthVerifyUser(
    ...
    group('purchase', () {
      test('logPurchaseCancelled constructs correctly', () {
        expect(
          () => analytics.logPurchaseCancelled(productId: 'test'),
          returnsNormally,
        );
      });
    ...
    group('screen', () {
      test('logScreenLegacyView constructs correctly', () {
        expect(
          () => analytics.logScreenLegacyView(legacyScreenCode: 'test'),
          returnsNormally,
        );
      });
      test('logScreenView constructs correctly', () {
        expect(
          () => analytics.logScreenView(screenName: 'test'),
          returnsNormally,
        );
      });
    });
